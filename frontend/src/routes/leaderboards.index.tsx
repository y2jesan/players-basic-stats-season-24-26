...
import { AGE_COLUMN, LEADERBOARD_CATEGORIES } from "@/lib/leaderboard-columns"
...
function LeaderboardsIndexPage() {
  ...
  const { data: leaderboards, isLoading } = useQuery({
    queryKey: ["football-leaderboards", season, qualified, young],
    queryFn: () =>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {LEADERBOARD_CATEGORIES.map(({ category, title, columns }) => (
          <LeaderboardCard
            ...
            isLoading={isLoading}
            columns={young ? [...columns, AGE_COLUMN] : columns}
          />
        ))}
      </div>
    </div>
